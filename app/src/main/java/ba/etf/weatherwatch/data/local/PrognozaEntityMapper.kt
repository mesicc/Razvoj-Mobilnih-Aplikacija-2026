...
object PrognozaEntityMapper {
    ...
    fun entityUPrognoza(entity: PrognozaEntity): Prognoza {
        val satnaType  = object : TypeToken<List<SatnaPrognoza>>() {}.type
        val dnevnaType = object : TypeToken<List<DnevnaPrognoza>>() {}.type

        val satna:  List<SatnaPrognoza>  = gson.fromJson(entity.prognozaPoSatimaJson, satnaType)  ?: emptyList()
        val dnevna: List<DnevnaPrognoza> = gson.fromJson(entity.prognozaDaniJson, dnevnaType) ?: emptyList()
    ...
    fun prognozaUEntity(prognoza: Prognoza): PrognozaEntity {
        return PrognozaEntity(
            ...
            vrijemeAzuriranja    = System.currentTimeMillis()
        )
    }
}